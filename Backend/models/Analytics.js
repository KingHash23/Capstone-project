  ...
  static async getSystemMetrics() {
    try {
      ...
      const [applicationStats] = await db.promise().query(`
        SELECT 
          COUNT(*) as total,
          SUM(CASE WHEN applications.status = 'accepted' THEN 1 ELSE 0 END) as accepted,  -- ✅ Fixed table reference
      ...
      const [matchMetrics] = await db.promise().query(`
        SELECT 
          AVG(CASE WHEN applications.status = 'accepted' THEN 1 ELSE 0 END) as acceptance_rate,  -- ✅ Fixed ambiguity
          COUNT(DISTINCT job_id) as jobs_with_applications,
          COUNT(DISTINCT job_seeker_id) as active_job_seekers
        FROM applications
        WHERE created_at >= DATE_SUB(NOW(), INTERVAL 30 DAY)
      ...
      if (role === 'job_seeker') {
        ...
        const [applications] = await db.promise().query(`
          SELECT 
            COUNT(*) as total_applications,
            COUNT(DISTINCT jobs.company_id) as companies_applied,
            SUM(CASE WHEN applications.status = 'accepted' THEN 1 ELSE 0 END) as successful_applications,  -- ✅ Fixed ambiguity
            AVG(CASE WHEN applications.status = 'accepted' THEN 1 ELSE 0 END) as success_rate
          FROM applications
          JOIN jobs ON applications.job_id = jobs.id
          WHERE job_seeker_id = ?
        `, [userId]);
        ...
        const [jobStats] = await db.promise().query(`
          SELECT 
            COUNT(*) as total_jobs,
            SUM(CASE WHEN jobs.status = 'open' THEN 1 ELSE 0 END) as active_jobs,  -- ✅ Fixed ambiguity
            COUNT(DISTINCT applications.job_seeker_id) as total_applicants
          FROM jobs
          LEFT JOIN applications ON jobs.id = applications.job_id
          WHERE company_id = (SELECT id FROM companies WHERE employer_id = ?)
        ...
        const [applicationStats] = await db.promise().query(`
          SELECT 
            COUNT(*) as total_applications,
            SUM(CASE WHEN applications.status = 'accepted' THEN 1 ELSE 0 END) as accepted_applications,  -- ✅ Fixed ambiguity
            AVG(CASE WHEN applications.status = 'accepted' THEN 1 ELSE 0 END) as acceptance_rate
          FROM applications
          JOIN jobs ON applications.job_id = jobs.id
          WHERE jobs.company_id = (SELECT id FROM companies WHERE employer_id = ?)
        `, [userId]);
    ...
    try {
      ...
      const [metrics] = await db.promise().query(`
        SELECT 
          COUNT(DISTINCT applications.job_seeker_id) as users_with_applications,
          COUNT(DISTINCT applications.job_id) as jobs_with_applications,
          AVG(CASE WHEN applications.status = 'accepted' THEN 1 ELSE 0 END) as recommendation_success_rate  -- ✅ Fixed ambiguity
        FROM applications
        WHERE created_at >= DATE_SUB(NOW(), INTERVAL 30 DAY)
      `);
